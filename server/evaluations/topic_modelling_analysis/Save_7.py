# Summary: Changed number of passes, does not affect the variety of words that come out as 
# associated words.
import csv
import pandas as pd
import gensim
...
#prints out the associated word list with the dataframe
print(df['associated_word'])

#prints out the full list of associated words
#print(df['associated_word'].tolist())

#prints out a dataframe of index number and associated word
with open('associated_words.txt', 'w') as file:
    for word in df['associated_word']:
        file.write(str(word) + '\n')